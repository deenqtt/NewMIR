...
NAV2D.Navigator = function (options) {
  ...
  // Add Global Costmap Layer (using OccupancyGrid)
  this.globalCostmapClient = new ROS2D.OccupancyGridClient({
    ros: ros,
    rootObject: this.rootObject,
    topic: "/keepout_filter_mask", // Use the keepout filter topic
    continuous: true,
    color: "#FF00FF", // Use pinkish-purple color as in RViz
  });

  // Rest of the NAV2D.Navigator code...
  ...
  ws.onmessage = function (event) {
    ...
    if (message.type === "goal_result") {
      ...
      console.log("Battery update received: ", message.level);
      batteryLevel.value = message.level; // Update battery level
    } else {
